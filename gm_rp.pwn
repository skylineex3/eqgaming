...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
		...
		else
		{
		    switch(listitem)
		    {
		        case 0:
		        {
		            ...
		            if(Item[uid][Active]) return Info(playerid, "Ten przedmiot nie może być aktywny.");
		            ...
		            if(IsPlayerInAnyVehicle(playerid))
		            {
						...
						cmd_me(playerid, "odkłada jakiś przedmiot w pojeździe.");
						return 1;
					}
		            ...
		            cmd_me(playerid, "odkłada jakiś przedmiot.");
				}
			}
		}
	}
	...
	if(dialogid == D_NEAREST_ITEMS)
	{
	    if(response)
	    {
			...
			if(Item[uid][Owner] != 0 && Item[uid][Owner] != PlayerData[playerid][char_uid]) return Info(playerid, "Nie możesz podnieść tego przedmiotu.");
			...
			if(Item[uid][in_vehicle] != 0 && IsPlayerInAnyVehicle(playerid))
			{
			    ...
			    cmd_me(playerid, "podnosi jakiś przedmiot ze schowka w pojeździe.");
			    return 1;
			}
			...
			cmd_me(playerid, "podnosi jakiś przedmiot z ziemi.");
		}
	}
	...
	if(dialogid == D_CREATE_ITEM)
	{
	    if(response)
	    {
			switch(listitem)
			{
			    case 0:{ ShowPlayerDialog(playerid, D_CT_CHANGE_NAME, DIALOG_STYLE_INPUT, "Tworzenie przedmiotu » Zmiana nazwy", "Podaj nową nazwę przedmiotu:", "Zmień", "Cofnij"); }
			    case 1:{ ShowPlayerDialog(playerid, D_CT_CHANGE_TYPE, DIALOG_STYLE_INPUT, "Tworzenie przedmiotu » Zmiana typu", "Podaj typ przedmiotu:", "Zmień", "Cofnij"); }
			    case 2:{ ShowPlayerDialog(playerid, D_CT_CHANGE_VAR_1, DIALOG_STYLE_INPUT, "Tworzenie przedmiotu » Zmiana wartości (1)", "Podaj nową wartość (1):", "Zmień", "Cofnij"); }
			    case 3:{ ShowPlayerDialog(playerid, D_CT_CHANGE_VAR_2, DIALOG_STYLE_INPUT, "Tworzenie przedmiotu » Zmiana wartości (2)", "Podaj nową wartość (2):", "Zmień", "Cofnij"); }
			    case 4:{ ShowPlayerDialog(playerid, D_CT_CHANGE_VAR_3, DIALOG_STYLE_INPUT, "Tworzenie przedmiotu » Zmiana wartości (3)", "Podaj nową wartość (3):", "Zmień", "Cofnij"); }
			    case 5:{ ShowPlayerDialog(playerid, D_CT_CHANGE_VAR_4, DIALOG_STYLE_INPUT, "Tworzenie przedmiotu » Zmiana wartości (4)", "Podaj nową wartość (4):", "Zmień", "Cofnij"); }
			    case 6:{ ShowPlayerDialog(playerid, D_CT_CHANGE_VAR_5, DIALOG_STYLE_INPUT, "Tworzenie przedmiotu » Zmiana wartości (5)", "Podaj nową wartość (5):", "Zmień", "Cofnij"); }
			    case 7:{ ShowPlayerDialog(playerid, D_CT_CHANGE_VAR_6, DIALOG_STYLE_INPUT, "Tworzenie przedmiotu » Zmiana wartości (6)", "Podaj nową wartość (6):", "Zmień", "Cofnij"); }
				case 8:{ ShowPlayerDialog(playerid, D_CT_CHANGE_OWNER, DIALOG_STYLE_INPUT, "Tworzenie przedmiotu » Zmiana ownera", "Podaj UID postaci aby nadać ownera:", "Zmień", "Cofnij"); }
				case 9:
				{
				    new uid = FreeID[playerid];
				    Item[uid][UID] = uid;
				    ...
				    Tip(playerid, "Przedmiot został utworzony.");
				}
			}
		}
	}
	...
	if(dialogid == D_CT_CHANGE_NAME)
	{
	    if(!response)
	    {
	        new uid = FreeID[playerid];
	        new StrinG[1000];
			format(StrinG, sizeof(StrinG), "Ustawienia\tWartości\nNazwa przedmiotu:\t%s\nTyp przedmiotu:\t%d\nWartość (1):\t%d\nWartość (2):\t%d\nWartość (3):\t%d\n\
				Wartość (4):\t%d\nWartość (5):\t%d\nWartość (6):\t%d\nOwner:\t%d\nZatwierdź, stwórz przedmiot", Item[uid][Name], Item[uid][Type], Item[uid][Var][0], Item[uid][Var][1], Item[uid][Var][2], Item[uid][Var][3], Item[uid][Var][4], Item[uid][Var][5], Item[uid][Owner]);

			ShowPlayerDialog(playerid, D_CREATE_ITEM, DIALOG_STYLE_TABLIST_HEADERS, "Tworzenie przedmiotu", StrinG, "Zatwierdź", "Anuluj");
		}
		else
		{
		    new uid = FreeID[playerid];
	...
	if(dialogid == D_CREATE_VEHICLE)
	{
	    if(response)
	    {
	        switch(listitem)
	        {
	            case 0:{ ShowPlayerDialog(playerid, D_VEHICLE_CN, DIALOG_STYLE_INPUT, "Tworzenie pojazdu » Zmiana pojazdu", "Podaj nową nazwę pojazdu:", "Zmień", "Cofnij"); }
	            case 1:{ ShowPlayerDialog(playerid, D_VEHICLE_CHP, DIALOG_STYLE_INPUT, "Tworzenie pojazdu » Zmiana HP", "Podaj nową wartość HP pojazdu:", "Zmień", "Cofnij"); }
	            case 2:{ ShowPlayerDialog(playerid, D_VEHICLE_CK1, DIALOG_STYLE_INPUT, "Tworzenie pojazdu » Zmiana koloru (1)", "Podaj nowy kolor pojazdu (1):", "Zmień", "Cofnij"); }
	            case 3:{ ShowPlayerDialog(playerid, D_VEHICLE_CK2, DIALOG_STYLE_INPUT, "Tworzenie pojazdu » Zmiana koloru (2)", "Podaj nowy kolor pojazdu (2):", "Zmień", "Cofnij"); }
	            case 4:{ ShowPlayerDialog(playerid, D_VEHICLE_CO, DIALOG_STYLE_INPUT, "Tworzenie pojazdu » Zmiana ownera", "Podaj UID postaci która ma być właścicielem pojazdu:", "Zmień", "Cofnij"); }
	            case 5:
	            {
	                new uid = FreeID[playerid];
	...
	if(dialogid == D_VEHICLE_CN)
	{
	    if(!response)
	    {
			...
			ShowPlayerDialog(playerid, D_CREATE_VEHICLE, DIALOG_STYLE_TABLIST_HEADERS, "Tworzenie pojazdu", StrinG, "Zatwierdź", "Anuluj");
			return 1;
		}
	...
	if(dialogid == D_ITEM_DISC)
	{
		...
		{
			new uid = FreeID[playerid];
			format(Item[uid][Name], 64, "Płyta CD: %s", inputtext);

		    ShowPlayerDialog(playerid, D_ITEM_DISC2, DIALOG_STYLE_INPUT, "Tworzenie płyty » Link", "Wprowadź link do audio streama by nagrać płytę:", "Stwórz", "Zamknij");
		}
	}
	...
	if(dialogid == D_ITEM_DISC2)
	{
	    if(response)
	    {
			...
			Tip(playerid, "Płyta została stworzona.");
		}
	}
	...
	if(dialogid == D_AREA)
	{
	    if(response)
	    {
	        switch(listitem)
	        {
	            ...
	            case 2:{ ShowPlayerDialog(playerid, D_AREA_CHANGE_OWNER, DIALOG_STYLE_INPUT, "Strefa » Zmiana właściciela", "Wprowadź UID postaci która ma być nowym właścicielem:", "Zatwierdź", "Cofnij"); }
				case 3:{ ShowPlayerDialog(playerid, D_AREA_CHANGE_HOUSE_COST, DIALOG_STYLE_INPUT, "Strefa » Zmiana ceny budynku mieszkalnego", "Wprowadź cenę (bez $) dla budynków mieszkalnych:", "Zatwierdź", "Cofnij"); }
				case 4:{ ShowPlayerDialog(playerid, D_AREA_CHANGE_BUISNESS_COST, DIALOG_STYLE_INPUT, "Strefa » Zmiana ceny budynku biznesowego", "Wprowadź cenę (bez $) dla budynków biznesowych:", "Zatwierdź", "Cofnij"); }
				case 5:{ ShowPlayerDialog(playerid, D_AREA_CHANGE_SIZE, DIALOG_STYLE_INPUT, "Strefa » Zmiana metrażu dla budynków", "Wprowadź nowy metraż który będzie minimalnym metrażem dla budynków:", "Zatwierdź", "Cofnij"); }
			}
		}
		else
		{
	...
	if(dialogid == D_BANK)
	{
	    if(response)
	    {
			switch(listitem)
			{
			    ...
			    {
			        new StrinG[500];
					format(StrinG, sizeof(StrinG), "{CCCCCC}Stan konta {FFFFFF}({00FF00}${FFFFFF}%d)\n{CCCCCC}Wpłać gotówkę\n{CCCCCC}Wypłać gotówkę\n{CCCCCC}Wykonaj przelew na konto\n{CCCCCC}Spłać należności {FFFFFF}({00FF00}${FFFFFF}%d)", PlayerData[playerid][char_bank], PlayerData[playerid][char_debt]);
			    	ShowPlayerDialog(playerid, D_BANK, DIALOG_STYLE_LIST, "Bankomat", StrinG, "Wybierz", "Anuluj");
				}
				case 1:{ ShowPlayerDialog(playerid, D_BANK_WPLAC, DIALOG_STYLE_INPUT, "Bankomat » Wpłata pieniędzy", "Wprowadź kwotę jaką chcesz wpłacić na swoje konto bankowe:", "Zatwierdź", "Cofnij"); }
				case 2:
				{
				    if(PlayerData[playerid][char_debt] != 0)
				    {
				        Tip(playerid, "Konto zablokowane. Najpierw spłać swoje należności!");
				        new StrinG[500];
						format(StrinG, sizeof(StrinG), "{CCCCCC}Stan konta {FFFFFF}({00FF00}${FFFFFF}%d)\n{CCCCCC}Wpłać gotówkę\n{CCCCCC}Wypłać gotówkę\n{CCCCCC}Wykonaj przelew na konto\n{CCCCCC}Spłać należności {FFFFFF}({00FF00}${FFFFFF}%d)", PlayerData[playerid][char_bank], PlayerData[playerid][char_debt]);
			    		ShowPlayerDialog(playerid, D_BANK, DIALOG_STYLE_LIST, "Bankomat", StrinG, "Wybierz", "Anuluj");
						return 1;
					}
					ShowPlayerDialog(playerid, D_BANK_WYPLAC, DIALOG_STYLE_INPUT, "Bankomat » Wypłata pieniędzy", "Wprowadź kwotę jaką chcesz wypłacić z bankomatu:", "Zatwierdź", "Cofnij");
				}
				case 3:
				{
				    if(PlayerData[playerid][char_debt] != 0)
					...
					ShowPlayerDialog(playerid, D_BANK_PRZELEW, DIALOG_STYLE_INPUT, "Bankomat » Przelew na inne konto", "Wprowadź numer konta oraz kwotę według przykładu: 3050432 500", "Zatwierdź", "Cofnij");
				}
				case 4:
				{
				    if(PlayerData[playerid][char_debt] == 0)
				    {
				        Tip(playerid, "Nie posiadasz żadnych należności.");
				        new StrinG[500];
						format(StrinG, sizeof(StrinG), "{CCCCCC}Stan konta {FFFFFF}({00FF00}${FFFFFF}%d)\n{CCCCCC}Wpłać gotówkę\n{CCCCCC}Wypłać gotówkę\n{CCCCCC}Wykonaj przelew na konto\n{CCCCCC}Spłać należności {FFFFFF}({00FF00}${FFFFFF}%d)", PlayerData[playerid][char_bank], PlayerData[playerid][char_debt]);
			    		ShowPlayerDialog(playerid, D_BANK, DIALOG_STYLE_LIST, "Bankomat", StrinG, "Wybierz", "Anuluj");
						return 1;
					}
					ShowPlayerDialog(playerid, D_BANK_DEBT, DIALOG_STYLE_LIST, "Bankomat » Spłata należności", "test", "Zatwierdź", "Cofnij");
				}
			}
		}
	}
	...
	if(dialogid == D_BANK_WPLAC)
	{
	    if(response)
	    {
	        if(!IsNumeric(inputtext)) return ShowPlayerDialog(playerid, D_BANK_WPLAC, DIALOG_STYLE_INPUT, "Bankomat » Wpłata pieniędzy", "Wprowadź kwotę jaką chcesz wpłacić na swoje konto bankowe:", "Zatwierdź", "Cofnij");
	        if(PlayerData[playerid][char_cash] < strval(inputtext))
	        {
	            Tip(playerid, "Nie posiadasz wystarczającej ilości gotówki przy sobie.");

	            ShowPlayerDialog(playerid, D_BANK_WPLAC, DIALOG_STYLE_INPUT, "Bankomat » Wpłata pieniędzy", "Wprowadź kwotę jaką chcesz wpłacić na swoje konto bankowe:", "Zatwierdź", "Cofnij");
	            return 1;
			}

			if(strval(inputtext) < 0 || strval(inputtext) > 1500000)
			{
			    Tip(playerid, "Wpisano niepoprawną kwotę.");
	...
	if(dialogid == D_BANK_WYPLAC)
	{
	    if(response)
	    {
	        if(!IsNumeric(inputtext)) return ShowPlayerDialog(playerid, D_BANK_WYPLAC, DIALOG_STYLE_INPUT, "Bankomat » Wypłata pieniędzy", "Wprowadź kwotę jaką chcesz wypłacić z bankomatu:", "Zatwierdź", "Cofnij");
            if(PlayerData[playerid][char_bank] < strval(inputtext))
	        {
	            Tip(playerid, "Nie posiadasz wystarczającej ilości gotówki na koncie.");
	...
	if(dialogid == D_BANK_PRZELEW)
	{
	    if(response)
	    {
			...
			else
			{
				...
				new str[128];
				format(str, sizeof(str), "Otrzymałeś przelew z numeru konta %d. Dostępne środki: $%d", PlayerData[playerid][char_bank_number], PlayerData[player][char_bank]);
				Tip(player, str);
	        ...
	        switch(listitem)
	        {
	            case 0:
	            {
					if(bonnet == VEHICLE_PARAMS_ON)
					{
					    ...
					    new str[64];
					    format(str, sizeof(str), "zamyka maskę w pojeździe %s", VehicleNames[GetVehicleModel(vehicleid) - 400]);
					    cmd_me(playerid, str);
					}
					else
					{
					    ...
					    new str[64];
					    format(str, sizeof(str), "otwiera maskę w pojeździe %s", VehicleNames[GetVehicleModel(vehicleid) - 400]);
					    cmd_me(playerid, str);
					}
	            }
	            case 1:
	            {
	                if(boot == VEHICLE_PARAMS_ON)
	                {
					    ...
					    new str[64];
					    format(str, sizeof(str), "zamyka bagażnik w pojeździe %s", VehicleNames[GetVehicleModel(vehicleid) - 400]);
					    cmd_me(playerid, str);
					}
					else
					{
					    ...
					    new str[64];
					    format(str, sizeof(str), "otwiera bagażnik w pojeździe %s", VehicleNames[GetVehicleModel(vehicleid) - 400]);
					    cmd_me(playerid, str);
					}
				}
				case 2:
				...
				case 3:
				{
				    ...
				    if(driver == 0 && passenger == 0)
				    {
				        ...
				        new str[64];
					    format(str, sizeof(str), "zamyka szyby w pojeździe %s", VehicleNames[GetVehicleModel(vehicleid) - 400]);
					    cmd_me(playerid, str);
				    }
				    else if(driver == 1 && passenger == 1)
				    {
				        ...
				        new str[64];
					    format(str, sizeof(str), "otwiera szyby w pojeździe %s", VehicleNames[GetVehicleModel(vehicleid) - 400]);
					    cmd_me(playerid, str);
				    }
				}
				case 4:
				...
				case 5:
				{
					...
					if(!strlen(list_players))
					    Info(playerid, "W pobliżu Twojej postaci nie znaleziono żadnych graczy.");
				    else
				    	ShowPlayerDialog(playerid, D_VEHICLE_SELL, DIALOG_STYLE_LIST, "Lista osób w pobliżu", list_players, "Wybierz", "Cofnij");
				}
				case 6:
				{
				    new list_players[256];
					...
					if(!strlen(list_players))
					    ...
					    ShowPlayerDialog(playerid, D_VEHICLE_GIVE, DIALOG_STYLE_LIST, "Lista osób w pobliżu", list_players, "Wybierz", "Cofnij");
				}
			}
		}
	}
	...
	if(dialogid == D_VEHICLE_SELL)
	{
	    if(response)
	    {
	        ...
	        new str[500];
	        format(str, sizeof(str), "Oferta sprzedaży pojazdu\nKupujący: %s (ID: %d)\nPrzebieg: %.1f\nPaliwo: %.1f\n\nWpisz kwotę za którą chcesz sprzedać pojazd i kliknij \"Sprzedaj\".", PlayerData[id][char_name], id, Vehicle[uid][veh_mileage], Vehicle[uid][veh_fuel]);

	        PlayerData[playerid][char_offer_id] = id;

			ShowPlayerDialog(playerid, D_VEHICLE_SELL2, DIALOG_STYLE_INPUT, "Sprzedaż pojazdu", str, "Sprzedaj", "Anuluj");
		}
	...
	if(dialogid == D_VEHICLE_SELL2)
	{
	    if(response)
	    {
			...
			new str[500];
	        format(str, sizeof(str), "Oferta kupno pojazdu\nSprzedający: %s (ID: %d)\nPrzebieg: %.1f\nPaliwo: %.1f\nCena: $%d\n\nW okienku wpisz \"potwierdzam\" aby zatwierdzić kupno pojazdu.", PlayerData[playerid][char_name], playerid, Vehicle[uid][veh_mileage], Vehicle[uid][veh_fuel], strval(inputtext[0]));
	...
	if(dialogid == D_VEHICLE_SELL3)
	{
	    ...
	    if(response)
	    {
	        if(!strcmp(inputtext[0], "potwierdzam", true))
	        {
	        	...
	        	{
	        	    new str[500];
	        		format(str, sizeof(str), "Oferta kupno pojazdu\nSprzedający: %s (ID: %d)\nPrzebieg: %.1f\nPaliwo: %.1f\nCena: $%d\n\nW okienku wpisz \"potwierdzam\" aby zatwierdzić kupno pojazdu.\n\n{FF0000}Nie posiadasz wystarczającej kwoty pieniędzy przy sobie.", PlayerData[id][char_name], id, Vehicle[uid][veh_mileage], Vehicle[uid][veh_fuel], PlayerData[id][char_offer_value]);
	...
	if(dialogid == D_VEHICLE_GIVE)
	{
		...
		if(response)
		{
			new str[500], id = strval(inputtext[0]), vehicleid = GetPlayerVehicleID(playerid), uid = GetVehicleUID(vehicleid);
 			format(str, sizeof(str), "Oferta przekazania pojazdu\nPrzekazujący: %s (ID: %d)\nPrzebieg: %.1f\nPaliwo: %.1f\n\nW okienku wpisz \"potwierdzam\" aby zatwierdzić przekazanie pojazdu.", PlayerData[playerid][char_name], playerid, Vehicle[uid][veh_mileage], Vehicle[uid][veh_fuel]);
...
public OnPlayerText(playerid, text[])
{
	if(PlayerData[playerid][char_bw] > 0)
	{
	    Info(playerid, "W tej chwili nie możesz korzystać z czatów InCharacter.");
	    return 0;
	}
	...
	if(text[0] == '.')
	{
	    ...
	    {
	        Info(playerid, "Posiadasz aktywną blokadę czatów OOC.");
	        return 0;
		}
    ...
    if(text[0] == '?')
	{
		cmd_me(playerid, "robi pytajacą minę.");
	    return 0;
	}
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	if(newkeys & KEY_YES)
	{
	    if(PlayerData[playerid][char_stage] == STAGE_ZONE_SET_X)
	    {
	    	...
	    	Tip(playerid, "Pozycja została zapisana.");
		}
		...
		if(GetNearestObject(playerid, 3.0) != -1)
		{
		    if(PlayerData[playerid][char_bw] != 0)
			{
				Info(playerid, "Nie możesz teraz skorzystać z tej komendy.");
				return 1;
			}
	...
	if(newkeys & KEY_CTRL_BACK)
	{
	    if(PlayerData[playerid][char_stage] == STAGE_ZONE_SAVE)
	    {
	        ...
	        Tip(playerid, "Strefa została zapisana w bazie danych.");
		}
	}
...
stock LoadObjectMmat()
{
	...
	printf("[load][mmat] Załadowano %d tekstur i %d tekstów na obiekcie.", loadedo, loadedt);
	return 1;
}
