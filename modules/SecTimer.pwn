...
public SecTimer()
{
	...
	{
	    new text[64];
	    if(hour == 0) format(text, sizeof(text), "* Dzwony w ratuszu biją 24 razy. *");
	    else format(text, sizeof(text), "* Dzwony w ratuszu biją %d razy. *", hour);
			...
			if(IsPlayerInAnyVehicle(i) && GetPlayerVehicleSeat(i) == 0)
			{
				...
				if(engine == VEHICLE_PARAMS_ON)
				{
				    ...
				    if(Vehicle[uid][veh_fuel] < 0.1)
				    {
				        ...
				        new strr[128];
				        format(strr, sizeof(strr), "W pojeździe %s kończy się paliwo.", VehicleNames[GetVehicleModel(vehid) - 400]);
				        cmd_do(i, strr);
					}
				}
			}
